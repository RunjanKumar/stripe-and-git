...
const { MESSAGES, ERROR_TYPES } = require('../Utils/constant');
...
const joiValidatorMethod = async (request, route) => {
    ...
    if (route.joiSchemaForSwagger.body && Object.keys(route.joiSchemaForSwagger.body).length) {
        request.body = await Joi.object(route.joiSchemaForSwagger.body).unknown().validate(request.body);
    ...
    if (route.joiSchemaForSwagger.headers && Object.keys(route.joiSchemaForSwagger.headers).length) {
        const headersObject = await Joi.object(route.joiSchemaForSwagger.headers).unknown().validate(request.headers);
...
let getHandlerMethod = (route) => {
    ...
    return (request, response) => {
        let payload = {};
        if (CONSTANTS.STRIPE_WEBHOOK_ENDPOINTS.includes(request.url)) {
            payload = {
                ...(request.body || {}),
...
let createSwaggerUIForRoutes = (app, routes = []) => {
    ...
    const swaggerOptions = {
        customSiteTitle: 'Miniheart API Documentations',
